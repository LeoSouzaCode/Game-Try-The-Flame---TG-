...
public class AnimacaoTabuleiro {
    ...
    public void animarMovimentoJogadorComEsteira(int antigaLinha, int antigaColuna, int novaLinha, int novaColuna, Runnable aoFinalizar) {
        ...
        jogador.addAction(
            Actions.sequence(
                ...
                Actions.delay(esteiraDuracao * 0.75f),
    ...
    private void animarEsteiraBonita(int antigaLinha, int antigaColuna, int novaLinha, int novaColuna, float duracao) {
        int dx = novaColuna - antigaColuna;
        int dy = novaLinha - antigaLinha;

        float passoX = (cartaLargura + espaco) * 1.18f;
        float passoY = (cartaAltura + espaco) * 1.18f;

        if (dx != 0) {

            for (int j = 0; j < Tabuleiro.COLUNAS; j++) {
                Cartas carta = cartasVisuais[antigaLinha][j];
            ...
            for (int i = 0; i < Tabuleiro.LINHAS; i++) {
                Cartas carta = cartasVisuais[i][antigaColuna];
                if (carta == null) continue;
    ...
    private void animarCartaEsteiraComEntradaESaida(Cartas carta, float moveX, float moveY, float delay, float duracao, boolean cartaSaiDoTabuleiro) {
        ...
        if (moveX != 0f) {
            ...
            float destinoEntradaX = (moveX < 0)
                ? bordaDireita
                : bordaEsquerda;
            float destinoEntradaY;
            destinoEntradaY = inicioY;
        ...
        if (moveY != 0f) {
            ...
            float destinoEntradaY = (moveY < 0)
                ? bordaCima
                : bordaBaixo;

            float entradaX;
            entradaX = inicioX;
            float destinoEntradaX;
            destinoEntradaX = inicioX;
